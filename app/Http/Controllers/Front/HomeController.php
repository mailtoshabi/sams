...
public function updateProfile(Request $request)
{
    ...
    if (empty($customer->location_name)) {
    ...
    $messages = [
        ...
        'kitchen_id.required'         => 'Kitchen is required',
    ];

    $validated = $request->validate($rules, $messages);
    // $postal_code = get_postal_code($request->latitude, $request->longitude);
    $postal_code = empty($customer->location_name)? get_postal_code($request->latitude, $request->longitude):$request->postal_code;
    // $validated['postal_code'] = $postal_code;
    $validated['postal_code'] = $postal_code ?? $customer->postal_code ?? '';
    if (empty($customer->location_name)) {
