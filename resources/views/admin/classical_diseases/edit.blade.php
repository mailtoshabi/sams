...
@section('title') {{ __('Edit Link Classical Disease Fields') }} @endsection
@section('css')
...
@section('content')
    ...
    @slot('title') {{ 'Edit Link Classical Disease Fields' }} @endslot
    @endcomponent
    ...
    <div class="container mt-4">
        ...
        <form action="{{ route('admin.classical_diseases.update', $classicalDisease->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="division_id" id="division_id" class="form-select select2-ajax"
                                ...
                                @foreach($divisions as $division)
                                    <option value="{{ $division->id }}" {{ old('division_id', $classicalDisease->division_id) == $division->id ? 'selected' : '' }}>
                                        {{ $division->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="chapter_id" id="chapter_id" class="form-select select2-ajax"
                                ...
                                @foreach($chapters as $chapter)
                                    <option value="{{ $chapter->id }}" {{ old('chapter_id', $classicalDisease->chapter_id) == $chapter->id ? 'selected' : '' }}>
                                        {{ $chapter->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="medicine_type_id" id="medicine_type_id" class="form-select select2-ajax"
                                ...
                                @foreach($medicine_types as $mt)
                                    <option value="{{ $mt->id }}" {{ old('medicine_type_id', $classicalDisease->medicine_type_id) == $mt->id ? 'selected' : '' }}>
                                        {{ $mt->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="formulation_id" id="formulation_id" class="form-select select2-ajax"
                                ...
                                @foreach($formulations as $formulation)
                                    <option value="{{ $formulation->id }}" {{ old('formulation_id', $classicalDisease->formulation_id) == $formulation->id ? 'selected' : '' }}>
                                        {{ $formulation->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="medicine_id" id="medicine_id" class="form-select select2-ajax"
                                ...
                                @foreach($medicines as $medicine)
                                    <option value="{{ $medicine->id }}" {{ old('medicine_id', $classicalDisease->medicine_id) == $medicine->id ? 'selected' : '' }}>
                                        {{ $medicine->name }}
                                    </option>
                                @endforeach
                            </select>
